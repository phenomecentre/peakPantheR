...
peakTable_FitGauss[1,]  <- c(NA, NA, TRUE, 522.2000122, 522.2000122, 522.2000122, 3346.453, 3322.979, 3379.317, 25792525.445, 25768307.538, 889280, 1840, 0.05400865734, 541.2220410, 7.464513318, 897391.7370, 1, 0, 5, 540, 535, 545, 24, 60, 1, 0, 0.023362696095338677, 1.565, 27.508921444636144, 11, NA, 1.2967091657127203)
peakTable_FitGauss[2,]  <- c(NA, NA, TRUE, 496.2000122, 496.2000122, 496.2000122, 3385.577, 3362.102, 3409.051, 32873727.359, 32818664.007, 1128960, 1471, 0.07199871100, 566.3075619, 7.788151879, 1133465.7198, 2, 0, 5, 564, 559, 569, 68, 98, 1, 0, 0.024586860166611640, 0.000, 28.700624487918503, 11, NA, 2.1345129619692504)
peakTable_FitGauss[3,]  <- c(NA, NA, TRUE, 464.2000122, 464.2000122, 464.2000122, 3456.000, 3432.525, 3479.474, 10818326.613, 10818278.099, 380736, 380735, 0.04489731232, 610.6945623, 7.504713149, 381973.2736, 3, 0, 5, 610, 605, 615, 24, 54, 1, 0, 0.026281775128549539, 1.565, 27.656069615314664, 11, NA, 1.3741191650431608)
peakTable_FitGauss[4,]  <- c(NA, NA, TRUE, 536.2000122, 536.2000122, 536.2000122, 3704.827, 3682.918, 3729.867, 8519479.783, 8460371.578, 330176, 197, 0.07353430967, 769.5553377, 6.824007014, 324408.4744, 4, 0, 5, 768, 763, 773, 24, 54, 1, 0, 0.022752704030186234, 3.130, 25.147467808758392, 11, 1.1968986203515855, 1.3314781556202822)
peakTable_FitGauss[,1]  <- c("ID-1", "ID-2", "ID-3", "ID-4")
...
peakTable_noFitGauss[1,]  <- c(NA, NA, TRUE, 522.2000122, 522.2000122, 522.2000122, 3344.888, 3322.979, 3379.317, 25792525.445, 25768307.538, 889280, 1840, NA, NA, NA, NA, 1, 0, 5, 540, 535, 545, 24, 60, 1, 0, 0.023376160866574614, 0.00, NA, 11, NA, 1.4839997536872727)
peakTable_noFitGauss[2,]  <- c(NA, NA, TRUE, 496.2000122, 496.2000122, 496.2000122, 3382.447, 3362.102, 3409.051, 32873727.359, 32818664.007, 1128960, 1471, NA, NA, NA, NA, 2, 0, 5, 564, 559, 569, 68, 98, 1, 0, 0.024601030353423384, 3.13, NA, 11, NA, 2.7082909206059269)
peakTable_noFitGauss[3,]  <- c(NA, NA, TRUE, 464.2000122, 464.2000122, 464.2000122, 3454.435, 3432.525, 3479.474, 10818326.613, 10818278.099, 380736, 380735, NA, NA, NA, NA, 3, 0, 5, 610, 605, 615, 24, 54, 1, 0, 0.026296922148575364, 0.00, NA, 11, NA, 1.5506880253870328)
peakTable_noFitGauss[4,]  <- c(NA, NA, TRUE, 536.2000122, 536.2000122, 536.2000122, 3701.697, 3682.918, 3729.867, 8519479.783, 8460371.578, 330176, 197, NA, NA, NA, NA, 4, 0, 5, 768, 763, 773, 24, 54, 1, 0, 0.022765817240815486, 0.00, NA, 11, 1.4026668257511667, 1.7784490549015182)
peakTable_noFitGauss[,1]  <- c("ID-1", "ID-2", "ID-3", "ID-4")
...
test_that('with fitGauss, no peakStatistic, no getEICs, no plotEICsPath, no getAcquTime, no verbose', {
  ...
  result_singleFileSearch <- evaluate_promise(peakPantheR_singleFileSearch(singleSpectraDataPath, targetFeatTable, fitGauss=TRUE, peakStatistic=FALSE, getEICs=FALSE, plotEICsPath=NA, getAcquTime=FALSE, FIR=NULL, verbose=FALSE, snthresh=3, peakwidth=c(2,20)))
...
test_that('no fitGauss, with peakStatistic, no getEICs, no plotEICsPath, no getAcquTime, no verbose', {
  ...
  # results (output, warnings and messages)
  result_singleFileSearch <- evaluate_promise(peakPantheR_singleFileSearch(singleSpectraDataPath, targetFeatTable, fitGauss=FALSE, peakStatistic=TRUE, getEICs=FALSE, plotEICsPath=NA, getAcquTime=FALSE, FIR=NULL, verbose=FALSE, snthresh=3, peakwidth=c(2,20)))
...
test_that('no fitGauss, no peakStatistic, with getEICs, no plotEICsPath, getAcquTime, verbose', {
  ...
  # results (output, warnings and messages)
  result_singleFileSearch <- evaluate_promise(peakPantheR_singleFileSearch(singleSpectraDataPath, targetFeatTable, fitGauss=FALSE, peakStatistic=FALSE, getEICs=TRUE, plotEICsPath=NA, getAcquTime=TRUE, FIR=NULL, verbose=TRUE, snthresh=3, peakwidth=c(2,20)))
...
test_that('no fitGauss, no peakStatistic, no getEICs, with plotEICsPath (should switch getEICs automatically), no getAcquTime, verbose', {
  ...
  # results (output, warnings and messages)
  result_singleFileSearch <- evaluate_promise(peakPantheR_singleFileSearch(singleSpectraDataPath, targetFeatTable, fitGauss=FALSE, peakStatistic=FALSE, getEICs=FALSE, plotEICsPath=savePath, getAcquTime=FALSE, FIR=NULL, verbose=TRUE, snthresh=3, peakwidth=c(2,20)))
...
test_that('change centwave param with ... (change snthresh), no fitGauss, no peakStatistic, no getEICs, no plotEICsPath, no getAcquTime, no verbose', {
  ...
  expected_peakTable$cpdID[4]     <- 'ID-4'
  ...
  # results (output, warnings and messages)
  result_singleFileSearch <- evaluate_promise(peakPantheR_singleFileSearch(singleSpectraDataPath, targetFeatTable, fitGauss=FALSE, peakStatistic=FALSE, getEICs=FALSE, plotEICsPath=NA, getAcquTime=FALSE, FIR=NULL, verbose=FALSE, snthresh=20, peakwidth=c(2,20)))
...
test_that('only one targetFeature, with getEICs', {
  ...
  # results (output, warnings and messages)
  result_singleFileSearch <- evaluate_promise(peakPantheR_singleFileSearch(singleSpectraDataPath, oneTargetFeature, fitGauss=TRUE, peakStatistic=FALSE, getEICs=TRUE, plotEICsPath=NA, getAcquTime=FALSE, FIR=NULL, verbose=FALSE, snthresh=3, peakwidth=c(2,20)))
...
test_that('no targetFeatures on import', {
  ...
  # results (output, warnings and messages)
  result_singleFileSearch <- evaluate_promise(peakPantheR_singleFileSearch(singleSpectraDataPath, emptyTargetFeatures, fitGauss=FALSE, peakStatistic=FALSE, getEICs=FALSE, plotEICsPath=NA, getAcquTime=FALSE, FIR=NULL, verbose=TRUE))
...
test_that('no features found without FIR', {
  # Empty targetFeatures
  ...
  # results (output, warnings and messages)
  result_singleFileSearch <- evaluate_promise(peakPantheR_singleFileSearch(singleSpectraDataPath, noMatchTargetFeatures, fitGauss=FALSE, peakStatistic=FALSE, getEICs=FALSE, plotEICsPath=NA, getAcquTime=FALSE, FIR=NULL, verbose=TRUE))
...
test_that('one feature not found with a FIR', {
  ...
  # Expected peakTable
  expected_peakTable              <- peakTable_noFitGauss[,c(3:27,1:2,28:33)]
  expected_peakTable[4,]          <- c(TRUE, 536.2, 536.1995, 536.2005, 3701.697, 3682.918, 3729.867, 8244658, 8244658, 330176, NA, NA, NA, NA, NA, NA, NA, NA, NA, NA, NA, NA, NA, NA, TRUE, 4, NA, NA, NA, NA, NA, NA, NA)
  expected_peakTable$cpdID[4]     <- 'ID-4'
  expected_peakTable$cpdName[4]   <- 'testCpd 4'
  expected_peakTable$is_filled[4] <- 1
  expected_peakTable$found        <- sapply(expected_peakTable$found, as.logical)
  expected_peakTable$is_filled    <- sapply(expected_peakTable$is_filled, as.logical)
  expected_peakTable$tailingFactor<- sapply(expected_peakTable$tailingFactor, as.logical)
  # Expected EICs
  expected_EICs       <- NULL
  # Expected acquTime
  expected_acquTime   <- NA
  ...
  # results (output, warnings and messages)
  result_singleFileSearch <- evaluate_promise(peakPantheR_singleFileSearch(singleSpectraDataPath, targetFeatTable, fitGauss=FALSE, peakStatistic=TRUE, getEICs=FALSE, plotEICsPath=NA, getAcquTime=FALSE, FIR=input_FIR, verbose=FALSE, snthresh=20, peakwidth=c(2,20)))
...
test_that('raise errors', {
  ...
  expect_error(peakPantheR_singleFileSearch(singleSpectraDataPath, targetFeatTable, FIR=notADataFrame), "Check input, FIR must be a data.frame not character")
  # FIR has wrong number of rows
  expect_error(peakPantheR_singleFileSearch(singleSpectraDataPath, targetFeatTable, FIR=wrongNbrows), "Check input, FIR must have the same number of rows as targetFeatTable")
  # FIR columns are wrong
  expect_error(peakPantheR_singleFileSearch(singleSpectraDataPath, targetFeatTable, FIR=wrongCols), 'Check input, FIR must have "mzMin", "mzMax", "rtMin" and "rtMax" as columns')
})
