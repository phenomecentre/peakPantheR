...
test_that('default output, with plots and colours, verbose, no verbose', {
  # temporary file
  savePath1         <- tempdir()
  # clear temp folder
  ...
  result_save     <- evaluate_promise(outputAnnotationDiagnostic(input_annotation, saveFolder=savePath1, savePlots=TRUE, sampleColour=input_colour, verbose=TRUE))
  ...
  result_save2    <- evaluate_promise(outputAnnotationDiagnostic(input_annotation, saveFolder=savePath2, savePlots=TRUE, sampleColour=input_colour, verbose=FALSE)
  )
...
test_that('no plot saved, verbose', {
  ...
  # results (output, warnings and messages)
  result_save     <- evaluate_promise(outputAnnotationDiagnostic(input_annotation, saveFolder=savePath3, savePlots=FALSE, verbose=TRUE))
...
test_that('no data to plot, verbose', {
  # temporary file
  savePath4         <- tempdir()
  # clear temp folder
  ...
  # expected
  expected_path_CSV   <- file.path(savePath4, "annotationParameters_summary.csv")
  expected_path_plot1 <- file.path(savePath4, "cpd_1.png")
  expected_path_plot2 <- file.path(savePath4, "cpd_2.png")
  expected_CSV        <- data.frame(matrix(nrow=2,ncol=21,dimnames=list(c(), c('cpdID', 'cpdName', 'X', 'ROI_rt', 'ROI_mz','ROI_rtMin', 'ROI_rtMax', 'ROI_mzMin', 'ROI_mzMax', 'X', 'uROI_rtMin', 'uROI_rtMax', 'uROI_mzMin', 'uROI_mzMax', 'uROI_rt', 'uROI_mz', 'X', 'FIR_rtMin', 'FIR_rtMax', 'FIR_mzMin', 'FIR_mzMax'))))
  ...
  # results (output, warnings and messages)
  result_save     <- evaluate_promise(outputAnnotationDiagnostic(input_annotation, saveFolder=savePath4, savePlots=TRUE, verbose=TRUE))
  ...
  expect_equal(length(result_save$messages), 6)
  expect_equal(result_save$messages[2:6], expected_message)
})
