context('annotation_fit_summary_UI_helper()')

## Test the fit summary table ofr UI
...
test_that('with FIR', {
  ...
  # expected values
  expected_summary            <- data.frame(matrix(data=c(100.,0.,0.02337616,8.499586, 66.7,33.3,0.02460103,16.361353), nrow=2,ncol=4,dimnames=list(c('ID-1 - Cpd 1', 'ID-2 - Cpd 2'), c('Ratio peaks found (%)', 'Ratio peaks filled (%)', 'ppm error', 'RT deviation (s)')), byrow=TRUE), stringsAsFactors=FALSE)
  expected_summary[,1:4]      <- sapply(expected_summary[,1:4], as.numeric)
  colnames(expected_summary)  <- c('Ratio peaks found (%)', 'Ratio peaks filled (%)', 'ppm error', 'RT deviation (s)')

  # results (output, warnings and messages)
  summary   <- evaluate_promise(annotation_fit_summary_UI_helper(input_annotation))

  # Check summary produced
  expect_equal(summary$result, expected_summary, tolerance=1e-6)
})

test_that('without FIR', {
  # input
  ...
  # expected values
  expected_summary            <- data.frame(matrix(data=c(100,0,0.02337616,8.499586, 100,0,0.02460103,16.361353), nrow=2,ncol=4,dimnames=list(c('ID-1 - Cpd 1', 'ID-2 - Cpd 2'), c('Ratio peaks found (%)', 'Ratio peaks filled (%)', 'ppm error', 'RT deviation (s)')), byrow=TRUE), stringsAsFactors=FALSE)
  expected_summary[,1:4]      <- sapply(expected_summary[,1:4], as.numeric)
  colnames(expected_summary)  <- c('Ratio peaks found (%)', 'Ratio peaks filled (%)', 'ppm error', 'RT deviation (s)')
