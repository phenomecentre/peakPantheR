...
It therefore can be deployed on a LC-MS instrument to integrate a set of pre-defined features (_e.g. spiked standards_) as soon as the acquisition of a sample is completed.
...
In the following example we will target two pre-defined features in a single raw MS spectra file from the `r Biocpkg("faahKO")` package. For more details on the installation and input data employed, please consult the [Getting Started with peakPantheR](getting-started.html) vignette.
...
* `peakStatistic` if `TRUE` calculates additional peak statistics: _'ppm_error'_, _'rt_dev_sec'_, _'tailing factor'_ and _'asymmetry factor'_
...
* `verbose` if `TRUE` messages calculation progress, time taken and number of features found (_total and matched to targets_)
