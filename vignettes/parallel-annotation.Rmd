...
vignette: >
    ...
    %\VignetteDepends{peakPantheR,faahKO,pander,BiocStyle,BiocParallel}
...
## Distinction between the three "Region of Interest" slots
`peakPantheR` tracks three conceptually distinct _RT_ / _m/z_ windows per
compound.

- **`@ROI`** — Acts as a broader EIC _extraction envelope_. Defines what is
read from the raw file into `@dataPoints`. The initial data read will then be
stored in subsequent fits, and re-used to avoid intensive data I/O.
Widening `@ROI` invalidates the cache and forces reading from the LC-MS
files on the next annotation.
- **`@uROI`** — _integration window_. What `findTargetFeatures` searches
within. This slot will be updated by the user when reviewing the integration
boundaries and refitting. Must satisfy `uROI ⊆ ROI` so refits used the cached
data.
- **`@FIR`** — _fallback integration window_. When `useFIR = TRUE` and a
fit fails inside `@uROI`, the raw signal is integrated over `@FIR` and the
resulting row is flagged `is_filled = TRUE`. Must satisfy `FIR ⊆ ROI`.

There are also two related switches:

- `@useFIR` selects `useFIR = FALSE` propagates failed fits
as `NA`; `useFIR = TRUE` produces filled rows via numerical integration of the `@FIR`.
- `@useUROI` should be kept as `TRUE` after the initial data read and is
...
knitr::include_graphics("figures/parallelAnnotation.png")
```
...
```{r, out.width = "700px", echo = FALSE}
knitr::include_graphics("figures/parallelAnnotation_procedure.png")
```
...
parallel (if `nCores` > 1) and return the successful annotations
(`result$annotation`) and failures (`result$failures`):
```{r}
# annotate files serially
annotation_result <- peakPantheR_parallelAnnotation(init_annotation, nCores=1,
...
(`result$annotation`) and failures (`result$failures`):
```{r}
# annotate files in parallel using two workers
annotation_result <- peakPantheR_parallelAnnotation(init_annotation, nCores=2,
                                                    curveModel='skewedGaussian',
                                                    verbose=TRUE)
...
knitr::include_graphics(
    "figures/parallel_annotation_diagnostic_cpd1.png")
```
