...
#'
#' @family peakPantheR
getTargetFeatureStatistic <- function(rawSpec, targetFeatTable, foundFeatTable, usePreviousEICs=NA, verbose=FALSE) {
  ...
  if (dim(targetFeatTable)[1] != dim(foundFeatTable)[1]) {
	...
	if (all(!is.na(usePreviousEICs))) {
		if (!is.list(EICs)) {
			stop("usePreviousEICs is not a list of xcms::Chromatogram")
		}
		if (length(EICs) != dim(targetFeatTable)[1]) {
		...
		if (!methods::is(EICs[[1]], 'Chromatogram')) {
			stop("usePreviousEICs is not a list of xcms::Chromatogram")
		}
	}
