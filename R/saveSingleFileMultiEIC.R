...
#'
#' @param EICs (xcms::Chromatograms) \code{xcms::Chromatograms} (containing one entry per ROI, same order as rows in foundPeakTable)
...
#'
#' @return None
saveSingleFileMultiEIC   <- function(EICs, foundPeakTable, savePath, width=15, height=15, verbose=TRUE) {

  ## check input
  if (length(EICs) != dim(foundPeakTable)[1]) {
    stop("Number of Chromatogram in EICs (", length(EICs),") and features in foundPeakTable (", dim(foundPeakTable)[1], ") do not match!")
  }

  ## Generate each ROI plot
  p_all         <- vector("list", length(EICs))
  for (i in 1:length(EICs)) {
    p_all[[i]]  <- plotEICDetectedPeakwidth(list(MSnbase::Chromatograms(data=list(EICs[[i]]))), foundPeakTable$cpdID[i], foundPeakTable$cpdName[i], foundPeakTable$rt[i], foundPeakTable$rtmin[i], foundPeakTable$rtmax[i], foundPeakTable$mzmin[i], foundPeakTable$mzmax[i], ratio=0.85, sampleColour='black', verbose=verbose)
  }
