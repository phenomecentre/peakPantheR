...
## Generate/Load data on trigger (make 'annotation' reactive, depends on input button pressed)
# check input but doesn't validate inputs
annotation <- reactive({
  # when none are pressed, return nothing
  if(input$triggerImportNewAnnotation == 0 & input$triggerLoadPreviousAnnotation == 0) {return()}

  input$triggerImportNewAnnotation      # force reevaluation
  input$triggerLoadPreviousAnnotation   # force reevaluation

  isolate({
    ## New annotation
    if(input$triggerImportNewAnnotation != 0) {
      res_data <- tryCatch({            # catch the errors in the import functions and kill the app
    ...
    ## Load annotation
    } else if(input$triggerLoadPreviousAnnotation!=0) {
      # Load
      res_data <- tryCatch({              # catch the errors in the import functions and kill the app
  ...
  # cannot guarantee annotation is valid (no peakPantheRAnnotation validation on load)
  res_data
})
...
output$showImportResult <- renderUI({
  ...
  # import is successful
    ...
    tmp_text <- capture.output(show(annotation()))
